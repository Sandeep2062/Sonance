...
DEFAULT_GITHUB_URL = "https://github.com/Sandeep2062/Sonance"
...
def make_clean_query(title: str, artist: str) -> str:
...
def normalize_title(title: str) -> str:
    ...
    # Strip track numbers like '13. ', '01 - ', '1-02 ', '04_ '
    title = re.sub(r"^\s*(?:\d{1,3}[\.\-\s_]+|\d{1,2}-\d{1,2}[\.\-\s_]+|[A-Za-z]\d{1,2}[\.\-\s_]+)", "", title).strip()
...
def run_provider(query: str, provider: str, out_path: str, lang_code: str, want_synced: bool) -> bool:
    ...
    # Direct Genius scraper with browser headers to avoid Cloudflare 403 blocks
    if provider.lower() == "genius" and not want_synced:
        try:
            import lyrics_engine
            lyrics = lyrics_engine.fetch_genius_plain_lyrics(query)
    ...
    # Anti-Mismatch Verification on saved file
    if os.path.exists(out_path) and os.path.getsize(out_path) > 50:
        try:
            import lyrics_engine
            base_audio = os.path.splitext(out_path)[0]
            for ext in [".mp3", ".flac", ".m4a", ".ogg"]:
                audio_file = base_audio + ext
                if os.path.exists(audio_file):
                    meta = lyrics_engine.get_audio_metadata(audio_file)
                    if meta["duration"] > 15.0:
                        with open(out_path, "r", encoding="utf-8", errors="ignore") as lf:
                            content = lf.read()
                        is_valid, _ = lyrics_engine.verify_lyrics_match(content, meta["duration"], query)
                        if not is_valid:
                            try:
                                os.remove(out_path)
                            except:
        ...
        if os.path.exists(lrc):
            ...
            if existing_lrc_state == "plain" and config.get("upgrade_plain_to_synced", True):
                ...
                if should_upgrade:
                    ...
                    query = make_clean_query(title, inferred_artist)
...
def about():
    ...
    body = (
        ...
        "© 2024-2026 Sandeep Khadka — MIT License\n"
        "github.com/Sandeep2062/Sonance\n"
    )
